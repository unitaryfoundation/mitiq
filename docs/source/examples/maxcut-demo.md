...
```{code-cell} ipython3
def count_cuts(graph: list[tuple[int, int]], set_a: set, set_b: set):
...
```{code-cell} ipython3
def count_cuts_from_string(graph: list[tuple[int, int]], bitstring: str):
...
def qaoa_hamiltonian(graph: list[tuple[int, int]]) -> np.ndarray:
...
def qaoa_ansatz(graph: list[tuple[int, int]], params: list[float]) -> Circuit:
