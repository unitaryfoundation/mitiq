...
"""Standalone tests for deb module (avoid full mitiq import due to numpy issue)."""
...
spec = importlib.util.spec_from_file_location(
    "symmetrization", str(mitiq_root / "mitiq" / "experimental" / "deb" / "symmetrization.py")
)
symmetrization = importlib.util.module_from_spec(spec)
spec.loader.exec_module(symmetrization)

spec = importlib.util.spec_from_file_location(
    "sharpening", str(mitiq_root / "mitiq" / "experimental" / "deb" / "sharpening.py")
)
...
def test_construct_circuits_num_variants():
    ...
    variants = symmetrization.construct_circuits(circuit, num_variants=5, random_state=42)
...
def test_construct_circuits_pauli_layers():
    ...
    variants = symmetrization.construct_circuits(circuit, num_variants=10, random_state=42)
...
def test_sharpen_no_winner_fallback():
    ...
    sharpened = sharpening.sharpen(results, threshold=50)  # High threshold to force fallback
