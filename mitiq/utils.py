...
def _count_gate_arities(circuit: cirq.Circuit) -> dict[str, int]:
    """Counts gates in a circuit grouped by arity."""
...
def compare_cost(
    ...
    mitigated = [convert_to_mitiq(c)[0] for c in qem_circuits]

    base_counts = _count_gate_arities(base)
    total_counts = {"1q": 0, "2q": 0, "nq": 0}
    for circ in mitigated:
        counts = _count_gate_arities(circ)
