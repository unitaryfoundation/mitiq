...
Usage:
    python scripts/benchmark_pec.py --circuit ghz --n-qubits 4 --noise-level 0.01 --shots 8192
"""
...
from mitiq import MeasurementResult, Observable, PauliString, Executor
from mitiq.benchmarks import generate_ghz_circuit, generate_quantum_volume_circuit, generate_mirror_circuit
...
def noisy_executor(circuit: cirq.Circuit, noise_level: float = 0.01, shots: int = 8192) -> MeasurementResult:
    ...
    result = cirq.DensityMatrixSimulator().run(noisy_circuit, repetitions=shots)
...
def ideal_executor(circuit: cirq.Circuit) -> float:
    ...
    density_matrix = cirq.DensityMatrixSimulator().simulate(circuit).final_density_matrix
...
def run_qermit_pec(
    ...
    return {
        ...
        "runtime": 0,
        "note": "qermit PEC not yet implemented - requires dependency installation",
    }
...
def print_results_table(
    ...
    print(f"Gate counts: {gate_counts['single_qubit']} single-qubit, {gate_counts['two_qubit']} two-qubit")
    ...
    print("\n" + "-" * 100)
    print(f"{'Tool':<35} {'Mitigated Value':<20} {'Error':<15} {'Improvement':<15} {'Runtime (s)':<15}")
    print("-" * 100)
    ...
    for result in results:
        ...
        if mitigated is None:
            print(f"{tool:<35} {'N/A':<20} {'N/A':<15} {'N/A':<15} {runtime:<15.3f}")
            if "note" in result:
                print(f"  Note: {result['note']}")
            continue
        
        mitigated_error = abs(mitigated - ideal_value)
        improvement = noisy_error / mitigated_error if mitigated_error > 0 else float('inf')
        
        print(f"{tool:<35} {mitigated:<20.6f} {mitigated_error:<15.6f} {improvement:<15.4f} {runtime:<15.3f}")
    
    print("=" * 100)
    print("\nNote: PEC typically requires many more samples (shots) than ZNE or REM")
    print("due to the probabilistic nature of the technique. The runtime reflects")
    print("the increased circuit execution overhead.")


def main():
    parser = argparse.ArgumentParser(
        description="Benchmark Probabilistic Error Cancellation (PEC) techniques"
    )
    parser.add_argument(
        "--circuit",
        type=str,
    ...
    # Generate circuit
    print(f"Generating {args.circuit.upper()} circuit with {args.n_qubits} qubits...")
    if args.circuit == "ghz":
    ...
    noisy_value = noisy_executor(circuit, noise_level=args.noise_level, shots=args.shots)
    ...
    def executor(circuit: cirq.Circuit) -> MeasurementResult:
        return noisy_executor(circuit, noise_level=args.noise_level, shots=args.shots)
    
    # Run benchmarks
    results = []
    
    print(f"\nRunning Mitiq PEC (Local Depolarizing, {args.samples} samples)...")
    results.append(run_mitiq_pec_local_depolarizing(circuit, executor, observable, args.noise_level, args.shots, args.samples))
    
    print(f"Running Mitiq PEC ({args.samples // 2} samples)...")
    results.append(run_mitiq_pec_custom_samples(circuit, executor, observable, args.noise_level, args.shots, args.samples // 2))
    
    print(f"Running Mitiq PEC ({args.samples * 2} samples)...")
    results.append(run_mitiq_pec_custom_samples(circuit, executor, observable, args.noise_level, args.shots, args.samples * 2))
    
    print("Running Mitiq PEC (High Precision)...")
    results.append(run_mitiq_pec_high_precision(circuit, executor, observable, args.noise_level, args.shots))
    
    print("Running qermit PEC (placeholder)...")
    results.append(run_qermit_pec(circuit, executor, observable, args.noise_level, args.shots))
